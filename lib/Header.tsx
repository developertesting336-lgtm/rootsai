...
export default function Header({ activeRoute }: HeaderProps) {
  ...
  return (
    <>
      ...
      {/* Header */}
      <header
        className="sticky-header bg-roots-navy-900 text-white h-header-mobile md:h-header border-b border-roots-navy-800"
        role="banner"
      >
        <div className="max-w-container mx-auto px-4 md:px-6 h-full flex items-center justify-between">
          ...
          <Link
            href="/"
            className="flex-shrink-0 font-bold text-lg leading-none hover:text-roots-light transition-colors"
            aria-label="ROOTS-AI Home"
          >
            <svg
          ...
          <Link
            href="/assessment"
            className="hidden md:inline-flex btn btn-primary text-sm md:text-base"
          >
            Start Assessment
          </Link>
          ...
          <button
            ...
            className="lg:hidden p-2 hover:bg-roots-navy-800 rounded-lg transition-colors touch-target"
    ...
    <Link
      ...
      className={`text-sm font-medium transition-colors ${
        active
          ...
          : 'text-roots-light hover:text-white'
      }`}
