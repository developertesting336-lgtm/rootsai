...
export default function MobileDrawer({ open, onClose }) {
    ...
    return (<>
      {/* Backdrop */}
      <div className="fixed inset-0 bg-black/50 z-drawer" onClick={onClose} aria-hidden="true"/>

      {/* Drawer */}
      <div ref={drawerRef} className="fixed inset-y-0 left-0 w-full max-w-sm bg-roots-navy-900 text-white z-drawer overflow-y-auto" id="mobile-drawer" role="dialog" aria-modal="true" aria-label="Navigation menu">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-roots-navy-800">
          ...
          <button onClick={onClose} className="p-2 hover:bg-roots-navy-800 rounded-lg transition-colors touch-target" aria-label="Close menu">
        ...
        <div className="sticky bottom-0 p-4 border-t border-roots-navy-800 bg-roots-navy-900">
          <Link href="/assessment" className="btn btn-primary w-full text-center" onClick={onClose}>
